...
cur.execute(
    """CREATE TABLE IF NOT EXISTS PageMatches(
    ...
    for tag in tags:
        ...
        for tag in text.findAll(tag):
            ...
            for s in sentences:
                ...
                if "kbcs" in s.lower():
                    cur.execute(
                        """INSERT INTO PageMatches (page_id, url, sentence) VALUES (?, ?, ?)""",
                        (page_id, url_short, s),
                    )
                    conn.commit()
                    print("--------------")
...
while True:
    ...
    cur.execute(
        """SELECT id, url FROM Pagelinks WHERE sentences is NULL and html is not NULL and error is NULL ORDER BY RANDOM() LIMIT 1"""
    )
    try:
        row = cur.fetchone()
        page_id = row[0]
    ...
    if url.endswith("sydneybats.org.au"):
        url_short = url
    else:
        url_short = url.split(".au", 1)[1]
...
for s in sentences:
    if "kbcs" in s.lower():
        cur.execute(
            """INSERT INTO PageMatches (page_id, url, sentence) VALUES ('NA', 'Menu', ?)""", (s,)
        )
        conn.commit()
...
cur.execute(
    """INSERT INTO Pagelinks (url, sentences, html) VALUES ('Menu',? , 'NA')""", (head_count,)
)
conn.commit()
...
for s in sentences:
    if "kbcs" in s.lower():
        cur.execute(
            """INSERT INTO PageMatches (page_id, url, sentence) VALUES ('NA', 'Footer', ?)""", (s,)
        )
        conn.commit()
...
cur.execute(
    """INSERT INTO Pagelinks (url, sentences, html) VALUES ('Footer',? , 'NA')""", (foot_count,)
)
conn.commit()
